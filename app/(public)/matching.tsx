...
import { useOrdersStore } from '@/lib/public/ordersStore';
...
type MatchingParams = { providers?: string | string[] };
...
function MatchingHeader() {
  ...
  useEffect(() => {
    const ring = (v: Animated.Value, delay = 0) =>
      Animated.loop(
        Animated.sequence([
    ...
    <View testID="matching.header" className="px-4 pt-6 pb-3">
      ...
      <View className="mt-3 items-center justify-center">
        <View className="w-14 h-14 items-center justify-center">
          <Animated.View
            testID="matching.radar"
            style={[
              { position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, borderRadius: 9999, backgroundColor: '#3B82F6' },
              ringStyle(ring1),
            ]}
          />
          <Animated.View
            style={[
              { position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, borderRadius: 9999, backgroundColor: '#60A5FA' },
              ringStyle(ring2),
            ]}
          />
      ...
      <View className="flex-row justify-between items-start">
        ...
        {p.eta ? (
          <View className="px-3 py-1 rounded-full bg-blue-50">
            <Text className="text-blue-600 text-xs font-medium">{p.eta}</Text>
          </View>
        ) : null}
      </View>
      <View className="flex-row items-center gap-4 mt-2">
        <Text className="text-neutral-700">Rating {ratingLabel}</Text>
        <Text className="text-neutral-400">|</Text>
        <Text className="text-neutral-700">{p.distance ?? 'Nearby'}</Text>
      </View>
      <View className="flex-row items-center justify-between mt-3">
        <Text className="text-neutral-500">Vehicle {p.car ?? 'TBD'}</Text>
        <Text className="text-neutral-900 font-semibold">{p.price ?? ''}</Text>
      </View>
      {/* SAFE gradient inside Pressable */}
      <Pressable
        testID={`pub.match.pick-${p.id}`}
        onPress={() => onChoose(p)}
...
export default function MatchingScreen() {
  ...
  const onChoose = (p: Provider) => {
    ...
    const serviceTitle = checkout.service?.title ?? 'Service';
    const basePrice = checkout.service?.price ?? 0;
    const addonsTotal = checkout.addons.reduce((sum, addon) => sum + (addon.price ?? 0), 0);
    const price = basePrice + addonsTotal;
    const durationMin = checkout.service?.durationMin ?? 60;

    const providerSource = checkout.selectedProvider;
    const rawRating = providerSource?.rating ?? p.rating;
    let ratingValue: number | undefined;
    if (rawRating !== undefined) {
      const numericRating = typeof rawRating === 'number' ? rawRating : parseFloat(String(rawRating));
      if (!Number.isNaN(numericRating)) {
        ratingValue = Number(numericRating.toFixed(1));
      }
    }

    ordersStore.addActive({
      id: orderId,
      status: 'active',
      serviceTitle,
      price,
      ...
      vehicle: checkout.vehicle
        ? { makeModel: checkout.vehicle.makeModel, plate: checkout.vehicle.plate }
        : undefined,
      whenLabel: checkout.time?.whenLabel ?? checkout.time?.fallbackLabel,
      slotStart: checkout.time?.slotStart ?? undefined,
      slotEnd: checkout.time?.slotEnd ?? undefined,
      createdAt: now,
      provider: {
        id: providerSource?.id ?? p.id,
        name: providerSource?.name ?? p.name,
        rating: ratingValue,
        vehicle: providerSource?.car ?? p.car,
        plate: p.plate,
        etaLabel: providerSource?.eta ?? p.eta ?? p.etaLabel,
      },
    });

    checkout.clearOrder();
    setCreatedOrderId(orderId);
    router.push({ pathname: '/(public)/confirm', params: { id: orderId } });
  };
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      {createdOrderId ? (
        <Text testID="pub.match.createdOrderId" className="sr-only">
          {createdOrderId}
        </Text>
      ) : null}
    </View>
